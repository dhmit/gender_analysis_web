...
def by_date(corpus_id,
    ...
    by_date_results = _apply_result_filters(output,
                                            sort=sort,
                                            diff=diff,
                                            limit=limit,
...
def by_document(corpus_id,
    ...
    for document_id in doc_ids:
        output[Document.objects.values_list('title', flat=True).filter(pk=document_id)] = results_query.get()[
            document_id]

    by_document_results = _apply_result_filters(output,
                                                sort=sort,
                                                diff=diff,
                                                limit=limit,
                                                remove_swords=remove_swords)
...
def by_gender(corpus_id,
    ...
    if remove_swords:
        output = _do_remove_swords(output)
...
def by_metadata(corpus_id,
    ...
    for document_id, gender_token_counters in results_query.get().items():
        ...
        for gender_label in all_gender_labels:
            ...
            output[matching_field][gender_label] = _merge_token_counters([
                gender_token_counters[gender_label],
                output[matching_field][gender_label]
            ])

    by_metadata_results = _apply_result_filters(output,
                                                sort=sort,
                                                diff=diff,
                                                limit=limit,
                                                remove_swords=remove_swords)
...
def by_overlap(corpus_id, word_window):
    ...
    by_gender_dict = by_gender(corpus_id, word_window)

    for gender_label in all_gender_labels:
        sets_of_adjectives[gender_label] = set(
            by_gender_dict[gender_label].keys()
        )
    ...
    for adj in intersects_with_all:
        ...
        for gender_label in all_gender_labels:
            results_by_gender[gender_label] = by_gender_dict[gender_label][adj]
...
def _apply_result_filters(key_gender_token_counters,
                          diff=False,
                          sort=False,
                          limit=10,
                          remove_swords=False):
    ...
    for key, gender_token_counters in key_gender_token_counters.items():
        if remove_swords:
            output[key] = _do_remove_swords(gender_token_counters)
        else:
...
def _diff_gender_token_counters(gender_token_counters):
    """
    ...
    :return: a dictionary mapping a gender to the difference in frequencies
...
def _do_remove_swords(gender_token_counters):
    """
    ...
    >>> test = {'Male': token_frequency_1, 'Female': token_frequency_2}
    >>> _do_remove_swords(test).get('Male').get('own') is None
    True
    """
    output = {}
...
def _sort_gender_token_counters(gender_token_counters,
                                limit=10):
    """
...
def _merge_token_counters(token_counters):
    """
